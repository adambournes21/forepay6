...
import { fetchThreadName, fetchLastMessage, fetchLastMessageTime, fetchPaymentsInChat, fetchUsername, fetchPayment, deleteThread, fetchUsersInChat } from '../firebaseConfig';

export default function RemoveChatSummary(props) {
        ...
        useCallback(() => {
            fetchUsername().then((uname) => {
                ...
                fetchUsersInChat(threadID).then((users) => {
                    console.log("uname3", uname);
                    fetchPaymentsInChat(threadID).then((payments) => {
                        console.log("uname4", uname);
                        return Promise.all(
                            payments.map((pid) => {
            ...
            fetchLastMessage(threadID).then((m) => {
                if (m.length > 4 && m.substring(0, 4) == '!<>?') {
                    setLastMessage(username + ' new expense added');
                } else {
    ...
    return (
        ...
        <TouchableOpacity onPress={() => press()}
                    ...
                    height: 160, width: '94%', backgroundColor: '#FFCECE',
